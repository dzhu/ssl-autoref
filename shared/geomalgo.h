...
// returns point on line x0-x1 closest to point p
template <class num>
vector2d<num> point_near_line(const vector2d<num> x0, const vector2d<num> x1, const vector2d<num> p)
{
  vector2d<num> n, v;
...
// Returns true iff the line segment joining @a1 to @a2 intersects the line
// segment joining @b1 to @b2.
template <class num>
bool segment_intersects(const vector2d<num> a1, const vector2d<num> a2, const vector2d<num> b1, const vector2d<num> b2)
{
  // b1 and b2 must be on opposite sides of a1--a2
  const bool a_intersects_b = (b1 - a1).perpdot(a2 - a1) * (b2 - a1).perpdot(a2 - a1) <= 0;
  // a1 and a2 must be on opposite sides of b1--b2
  const bool b_intersects_a = (a1 - b1).perpdot(b2 - b1) * (a2 - b1).perpdot(b2 - b1) <= 0;

  return b_intersects_a && a_intersects_b;
}
...
// calc_circle
template <class num>
bool CalcCircle(vector2d<num> &cen, num &rad, const vector2d<num> p1, const vector2d<num> p2, const vector2d<num> p3)
{
  vector2d<num> a, b;
  num da, db;
...
template <class vector>
double ray_plane_intersect(vector pOrigin, vector pNormal, vector rOrigin, vector rVector)
{
  // FIXME: if they're parallel, this will fail!
  return ((dot(pNormal, pOrigin) - dot(pNormal, rOrigin)) / dot(pNormal, rVector));
...
bool ray_sphere_intersect(vector r0, vector rV, vector s0, vector sv, real sR, vector *p, real *t)
// intersect a ray (r0+rV*t) with a sphere with radius sR, at s0 and moving at velocity sv
// returns: p = the first point of intersection.
{
  vector p0 = r0 - s0;
  vector v0 = rV - sv;
  const real a = v0.sqlength();
  if (a < EPSILON) return (false);
  const real b = 2.0 * dot(p0, v0);
  const real c = p0.sqlength() - sR * sR;
...
template <class vector, class real>
bool CircleTangentDir(vector cen, real rad, vector p, vector &left, vector &right)
{
  vector delta, basis;
  vector tl, tr;
  double d2, l, t;
...
template <class vector, class real>
bool CircleTangent(vector cen, real rad, vector p, vector &left, vector &right)
{
  vector delta, basis;
  vector tl, tr;
  double d2, l, t;
